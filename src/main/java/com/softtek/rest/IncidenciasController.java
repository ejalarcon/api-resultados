...
public class IncidenciasController {
	...
	public void updateGrupo(@RequestBody Incidencias incidencias) {
		...
		List<Incidencias> inci = incidenciasRepository.findAllByGrupoAndEstado(incidencias.getGrupo(),
				incidencias.getEstado());
		if (inci.size() > 0) {
	...
	// method = RequestMethod.GET)
	// @ResponseBody
	// public List<Incidencias> findAllByEstadoAndId(@RequestParam("estado")
	// String estado, @RequestParam("_id") String _id){
	...
	// incidenciasRepository.findAllByGrupoAndEstado(grupo, estado);
